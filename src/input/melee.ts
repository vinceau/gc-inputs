...
function fromCardinals(centers: NumberPair, l: number, r: number, d: number, u: number) {
...
function renormaliseAxisInput([lx, ly]: NumberPair, [rx, ry]: NumberPair, [dx, dy]: NumberPair, [ux, uy]: NumberPair, [x, y]: NumberPair) : NumberPair {
  let invMat;
...
function scaleUnitToGCAxes (x: number, y: number) : NumberPair {
...
function scaleToGCAxes ( x: number, y: number
                       , stickCardinals : null | StickCardinals
                       , customCenterX: number, customCenterY: number) : NumberPair {
...
function unitRetract ( [x, y]: NumberPair ) : NumberPair {
...
function meleeAxesRescale ( [x,y]: NumberPair ) : NumberPair {
...
export function scaleToMeleeAxes ( x: number, y: number
                                 , isGC : boolean
                                 , stickCardinals : null | StickCardinals
                                 , customCenterX : number = 0, customCenterY : number = 0 ) : NumberPair {
  let xnew = x;
  let ynew = y;
...
export function tasRescale ( x: number, y: number, isDeadzoned = false) : NumberPair {
